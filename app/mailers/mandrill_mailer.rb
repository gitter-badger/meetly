...
class MandrillMailer
  ...
  def get_options(participant, db_days)
    ...
    if pdays.length > 0
      if pdays.length == 3
        ...
        options = "Dzień"
        options += " 1" if pdays.include?(db_days[0])
        options += " 2" if pdays.include?(db_days[1])
        options += " 3" if pdays.include?(db_days[2])
        puts options
      else
        options = "Dzień"
        options += " 1 i" if pdays.include?(db_days[0])
        options += " 2" if pdays.include?(db_days[1]) && pdays.include?(db_days[0])
        options += " 2 i" if pdays.include?(db_days[1]) && !pdays.include?(db_days[0])
        options += " 3" if pdays.include?(db_days[2])
        puts options
      end
    end

    options += " + Nocleg x #{participant.nights.length}" if participant.nights.length != 0
    options += " + Obiad x #{participant.dinners.length}" if participant.dinners.length != 0
    puts options
    options
  end
  ...
  def get_to_pay_text(participant)
    ...
    if participant.paid == participant.cost
      ...
      return 'Otrzymaliśmy o <strong>' + q + ' zł</strong> więcej niż wymagana kwota. <br /> Przy rejestracji w dniu przyjazdu na konferencję pieniądze zostaną Ci zwrócone. <br />'
    elsif participant.paid > participation_cost && participant.paid < participant.cost
      'Do opłacenia pełnego udziału brakuje <strong>' + qm + ' zł.</strong><br /> Za obiady i noclegi można zapłacić również na miejscu.<br /> W razie rezygnacji z obiadów lub noclegów, prosimy o kontakt pod adresem rejestracja@poczatek.org. <br />'
    else
      'Do opłacenia podstawowego kosztu udziału brakuje <strong>' + qm + ' zł.</strong><br /> Prosimy o dopłatę do wymaganej kwoty w ciągu <strong>2 dni roboczych</strong>.'
    end
  end
end
